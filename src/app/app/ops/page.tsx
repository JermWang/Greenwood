        ...
        <div className="telemetry-layout">
          <section className="telemetry-overview">
            ...
            <div className="telemetry-gauges">
              ...
              <StatCard label="GREEN / second" value={op.productionRate.toFixed(4)} tone="cyan" />
          ...
          <section className="eg-console-card telemetry-lines">
            ...
            <div className="mt-4 space-y-2">
              {op.nodes.length === 0 ? <p className="text-sm text-emerald-100/45">No open desks.</p> : op.nodes.map((node, index) => {
                const storage = node.storageCap > 0 ? Math.min(100, (node.pendingGreen / node.storageCap) * 100) : 0;
                return (
                  <div key={node.id} className="telemetry-line-row">
                    ...
                    <span className="text-right font-mono text-[9px] text-emerald-100/55">{node.productionRate.toFixed(4)}<br /><span className="text-[7px] text-lime-300">GREEN/S</span></span>
                  </div>
                );
              })}
            </div>
