// BNTY price discovery.
//
// Crates are priced in dollars, so the engine needs to know what BNTY is worth.
...
/**
 * Manual price override, in USD per BNTY.
 *
 * Until BNTY trades there is no market to read, and after it lists there is no
 ...
 *
 * A stale price is a real hazard: if BNTY 10x'd a week ago and nobody updated
...
export function getOsrUsdPrice(): PriceInfo {
  ...
  if (price == null || !Number.isFinite(price) || price <= 0) {
    return { usdPerBnty: null, setAtMs: null, ageMs: null, stale: true };
  }
  ...
  return { usdPerBnty: stale ? null : price, setAtMs: setAt, ageMs, stale };
}

export function setOsrUsdPrice(usdPerBnty: number) {
  if (!Number.isFinite(usdPerBnty) || usdPerBnty <= 0) {
    throw new Error('price must be a positive number');
  }
  setProtocolValue(PRICE_KEY, String(usdPerBnty));
